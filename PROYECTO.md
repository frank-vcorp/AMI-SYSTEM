...
| mod-citas | MOD-CITAS | FASE 1 – Flujo Principal | 1 | Frontend · Backend | done | 100 | Agenda de citas: CRUD + check-in + "Generar Expediente" button + serviceIds. 100% Completado. | mod-clinicas + mod-empresas |
| mod-expedientes | MOD-EXPEDIENTES | FASE 1 – Flujo Principal | 1 | Frontend · Backend | done | 100 | Flujo central: paciente → examen → expediente → validación. 100% Completado (CRUD + Exámenes + Estudios + 7 API routes + UI componentes). | MOD-CITAS, Prisma schema |
| mod-validacion | MOD-VALIDACION | FASE 1 – Flujo Principal | 1 | Backend · Data | done | 100 | IA inteligente lee estudios automáticamente. Médico valida y firma los resultados. 100% Completado (Panel + Semáforos + Firma digital + 4 API routes). | mod-expedientes + core-signatures |
| mod-reportes | MOD-REPORTES | FASE 1 – Flujo Principal | 1 | Backend · Frontend | done | 100 | Certificados de validación imprimibles. 100% Completado (CertificateViewer MVP + Print dialog + PDF export API + Delivery flow). | mod-validacion + core-storage |
